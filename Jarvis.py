    ...
    try:
        class Game:
            def __init__(self):
                ...
                pygame.display.set_caption('Jarvis typing test')

            def draw_text(self, screen, msg, y, fsize, color):
                font = pygame.font.Font(None, fsize)
                text = font.render(msg, 1, color)
                text_rect = text.get_rect(center=(round(self.w/2), round(y)))
            ...
            def show_results(self, screen):
                if(not self.end):
                    ...
                    screen.blit(round(self.time_img()), (round(self.w/2-75)), round(self.h-140))
        ...
        print(e)
        print("Something is just not working out")
def developer_mode():
...
while True:
    try:
        ...
        if 'typing' in inp:
            print("Prepare your fingers for a rough race on the keyboards circuit...")
            time.sleep(1.5)
            Jarvis_Typing_test()
        ...
        while ok:
            try:
                ...
                with sr.Microphone() as source:
                    ...
                    if 'typing' in text:
                        engine.say("Prepare your fingers for a rough race on the keyboards circuit...")
                        engine.runAndWait()
                        time.sleep(1)
                        Jarvis_Typing_test()
                    if 'clear' in text:
